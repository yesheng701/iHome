...
func init() {
	// 注册orm所关联的数据库
	// 注册一个orm所关联的数据库的驱动 最后一个参数是数据库最大链接个数
	orm.RegisterDataBase("default", "mysql", "root:ys!159357@tcp(127.0.0.1:3306)/ihome_go?charset=utf8", 30)

	// 给orm注册model模块 注册表
	orm.RegisterModel(new(User), new(House), new(OrderHouse), new(Area), new(HouseImage), new(Facility))

	// create table 第二个参数是否强制替换 第三个参数如果没有则创建
